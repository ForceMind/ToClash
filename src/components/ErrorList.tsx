...
function zhMessage(issue: ConversionIssue): string {
  const parameter = /Parameter "([^"]+)"/.exec(issue.message)?.[1]
  const plugin = /Plugin "([^"]+)"/.exec(issue.message)?.[1]
  if (issue.code === 'IGNORED_PARAMETER' && parameter) return `参数“${parameter}”当前无法映射到 Mihomo，已忽略。`
  if (issue.code === 'UNSUPPORTED_PLUGIN' && plugin) return `插件“${plugin}”可能不受 Mihomo 支持。`
  return zhMessages[issue.code]
}
