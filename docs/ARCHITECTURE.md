...
`CustomRouting` 支持 `mode`、`directDomains`、`proxyDomains`、29 项服务 `presets`、`bypassCgnat`（默认关闭），以及由后缀和 IP-literal UDP DNS 数组组成的 `intranet`。四项原有服务默认开启，新增 25 项默认关闭；核心 API 可为不同内网后缀指定不同 DNS，更具体后缀优先，同后缀不同 DNS 定义报错。
...
常规模式使用公共直连 DoH、GeoSite / GeoIP 和 `MATCH,PROXY`。默认直连模式的 `nameserver`、`direct-nameserver` 与 `proxy-server-nameserver` 都是 `system`；选中服务的 DNS 使用 `#FORCE_PROXY` 公共 DoH，显式内网 DNS 同时进入业务和节点引导策略，并通过 `direct-nameserver-follow-policy` 保留该优先级。该模式最后生成 `MATCH,DIRECT`。
...
- 初始：简体中文、跟随系统初始主题、完整配置、29 项预设中原四项开启、其余 25 项关闭，内网及 CGNAT 关闭。
...
- 自动恢复：v1 的旧五字段会补齐模式、服务预设和 CGNAT 默认值；新增的默认关闭服务缺失时保持关闭。未知字段忽略，损坏或未知版本提示失败且初次挂载不覆盖原值；编辑或显式重置后可以替换。输入继续由规则校验器验证。
- 保存：自定义、模式、服务开关、CGNAT 和内网输入变化时同步写入；空默认值删除本工具 key，不清理同源其他存储项。写入失败显示提示。
- 输出失败：不显示原始异常/堆栈，只显示安全提示。
- 复制/下载：仅显式点击触发。异步复制用修订号防止过期提示；下载移除临时元素并延迟释放 Blob URL。

不存在远程同步、后台轮询或系统设置变更。持久化只包含分流设置，不包含节点凭据、账号或输入的代理链接。临时 Blob URL 仅用于下载，不将输入写入页面 URL。
