...
把代理链接或已有 Clash / Mihomo YAML 转换、补全为可编辑的配置文件。打开网页、粘贴内容或导入 YAML、调整分流、下载配置，再导入你的代理客户端即可。
...
当前正式版本：**v0.3.6**。本地待发布版本为 **v0.3.7**，新增已有 Clash / Mihomo YAML 的直接粘贴与文件导入。首次访问会打开全套“新手模式”，通过六步弹窗完成网络模式、代理链接、常用服务、可选网站分流、可选内网 DNS 和最终转换；页头按钮可随时重新打开。已发布版本的证据见[v0.3.6 正式发布记录](docs/RELEASE_v0.3.6.md)。

## 第一次使用

1. 在“代理链接或 Clash YAML”中逐行粘贴代理链接，或直接粘贴完整 YAML；也可点击“导入 YAML 文件”。
...
导入后，ToClash 会保留 `proxies:` 中的节点原样，包括 VLESS `xhttp` 及其未知字段；也保留未由 ToClash 接管的顶层字段，例如 `tun`、`sniffer`、`rule-providers` 和额外 DNS 字段。页面会尽量恢复默认直连模式、已选服务、自定义直连 / 代理和使用同一 DNS 的内网后缀，方便继续编辑。

ToClash 会将运行模式设为 `rule`，并重新生成它管理的本机规则、服务规则、`FORCE_PROXY`（常规模式还包括 `PROXY` / `AUTO`）、相关 DNS policy 和最终 `MATCH`。原配置中不属于这些范围的规则保留在 ToClash 规则之后、最终 `MATCH` 之前。遇到多个内网后缀使用不同 DNS，或常规模式使用 `system` 的内网区域，页面会提示该部分暂不能用当前表单编辑，并保留原有规则和 DNS policy。

导入只在当前页面内存中生效。原始 YAML 和节点凭据不会写入 LocalStorage，也不会上传。导出前请阅读右侧 YAML；“仅 proxies”会刻意只导出节点，不保留 DNS、策略组、TUN 或规则。

导入校验 YAML 结构、节点名称和必要的 `type`，不会把未知节点协议或扩展字段重新解释为另一种格式；它们会原样保留。是否能被你的 Mihomo / Clash Verge 核心版本加载，仍需在客户端实际导入验证。

## 新手模式

首次访问时自动打开弹窗，也可以随时点击页头的“新手模式”。它是完整页面的简易入口，按顺序完成：

1. 选择常规分流或默认直连。
2. 粘贴至少一条有效代理链接。
3. 选择四项常用代理服务；其他服务选择保持不变，完成后仍可在完整页面调整全部 29 项。
4. 按需填写始终直连和始终代理的网站。
5. 按需启用并填写内网 DNS；字段保持空白，不自动填入任何后缀。
6. 查看摘要，点击“应用并转换”后一次性写回完整页面并生成 YAML。

关闭或按 Escape 不会应用向导草稿。浏览器只额外保存一个“已看过新手模式”的标记，不保存向导中的代理链接；页头按钮始终可手动重新打开。
...
填写域名会包含它的子域名；填写网址只按网站域名分流，不区分页面路径。不要把 `DOMAIN-SUFFIX,...` 粘贴到这个字段；需要编辑整份 YAML 时，使用页面顶部“代理链接或 Clash YAML”输入框。
...
不确定怎么填时，使用页头“新手模式”并进入“可选：内网 DNS”步骤。它不会把 `svc.cluster.local`、DNS 或任何其他配置作为默认值。页面上的占位文本只说明格式；重新打开时会显示你已填的内容，便于继续修改。
...
- **新手模式标记：** 只记录是否已经显示过首次引导，不包含节点、域名、DNS 或其他表单内容。
- **不自动保存：** 代理链接、导入的 YAML、节点凭据、生成的 YAML、语言、主题和输出格式。
