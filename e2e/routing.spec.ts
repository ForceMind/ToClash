...
test('real browser conversion, privacy, settings, download and keyboard workflow', async ({ page, context }, testInfo) => {
  ...
  await page.goto('/')
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('代理链接 → Clash YAML')
  await expect(page.locator('html')).toHaveAttribute('lang', 'zh-CN')
  expect((await page.locator('html').getAttribute('class') ?? '').split(' ').includes('dark')).toBe(testInfo.project.name === 'mobile-dark')
  ...
  expect(parse(await output.inputValue()).rules).toContain('DOMAIN-SUFFIX,openai.com,FORCE_PROXY')
  expect(Object.keys(parse(await output.inputValue()).dns['nameserver-policy']).slice(-2)).toEqual(['geosite:cn', 'geosite:geolocation-!cn'])
  ...
  expect(parse(await output.inputValue()).rules).not.toContain('DOMAIN-SUFFIX,openai.com,FORCE_PROXY')
  ...
  await page.getByLabel('第 1 步：始终直连', { exact: true }).fill('bank.example\n192.0.2.1')
  ...
  await expect(page.getByRole('heading', { name: '分流覆盖提示' })).toBeVisible()
  ...
  await page.screenshot({ path: testInfo.outputPath('validation-error.png'), fullPage: true })
  ...
  await page.locator('summary').filter({ hasText: '企业 / 家庭内网 DNS' }).click()
  ...
  expect(internalConfig.dns['nameserver-policy']['+.corp.example']).toEqual(['udp://192.0.2.53:53'])
  expect(internalConfig.dns['proxy-server-nameserver-policy']['+.corp.example']).toEqual(['udp://192.0.2.53:53'])
  expect(internalConfig.dns['proxy-server-nameserver-policy']['+.lan']).toEqual(['rcode://refused'])
  ...
  expect(parse(await output.inputValue()).rules).toContain('IP-CIDR,100.64.0.0/10,DIRECT,no-resolve')
  ...
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('Proxy links → Clash YAML')
  const wasDark = (await page.locator('html').getAttribute('class')) === 'dark'
  await page.getByRole('button', { name: wasDark ? 'Switch to light mode' : 'Switch to dark mode' }).click()
  expect((await page.locator('html').getAttribute('class')) === 'dark').toBe(!wasDark)
  await page.screenshot({ path: testInfo.outputPath('english.png'), fullPage: true, animations: 'disabled' })
