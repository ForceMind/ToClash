...
以下代理组、公共 DNS 和代理兜底说明适用于常规模式，默认直连模式以上述说明为准。

## 从哪里开始

1. 粘贴节点，点击“转换”；默认输出完整配置，原四项服务预设开启，新增服务默认关闭。
...
## 为什么同一个域名有 FORCE_PROXY 和 REJECT 两行？
...
| 行 | 含义 |
| --- | --- |
| 第一行 | `claude.com` 及其子域名使用 `FORCE_PROXY` 代理组，该组只提供代理节点或 AUTO |
| 第二行 | 同一请求若继续匹配到这里，就拒绝连接，防止落入后面的直连规则 |

规则从上往下匹配。正常选中第一行后，不会再执行第二行，所以不会把已经通过代理的请求再次拦截。

具体例子：UDP 请求遇到所选代理不支持 UDP 时，Mihomo 会继续向下匹配；紧接着的同条件 `REJECT` 会拒绝该请求。普通的代理超时或连接失败不等于重新匹配下一条规则，不能把这两行理解为连接失败回调。

这组规则由 ToClash 自动生成，一般保留两行即可；实际 YAML 中写 `FORCE_PROXY`，不要加入 Markdown 转义用的反斜杠。它只约束进入 Mihomo 并匹配到这里的流量；前面的本地 / 内网或自定义直连规则仍优先，也不能控制绕过客户端的流量。

依据：[Mihomo 路由规则优先级](https://wiki.metacubex.one/config/rules/#优先级)说明从上往下匹配，以及代理不支持 UDP 时继续匹配的行为；本项目生成逻辑位于 `src/core/rules/plan.ts`。
...
5. 已开启的服务预设，按服务目录顺序；同域名先匹配的规则优先。
...
v0.3.3 增加分类、搜索和独立开关，完整目录、来源和共享域名影响见[服务目录](SERVICE_CATALOG.md)。默认直连模式中所有选中服务使用 FORCE_PROXY；常规模式保留 Google / GitHub 的 PROXY 行为，其余选中服务使用 FORCE_PROXY。新增项默认关闭，旧存储补齐关闭值。
