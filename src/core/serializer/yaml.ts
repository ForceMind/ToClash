...
export function serializeMihomo(nodes: ProxyNode[], format: OutputFormat, routing: CustomRouting = {}): string {
  ...
  if (format === 'full') {
    const comments: Record<string, string> = {
      'proxy-groups': ' 代理组：PROXY 可手动直连；FORCE_PROXY 仅包含代理节点；AUTO 自动选择',
      dns: ' DNS：域名策略与分流保持一致；内网域名必须填写可达的内网 DNS',
    ...
    if (isMap(policy)) policy.commentBefore = ' 未配置内网 DNS 的保留本地后缀拒绝上游查询，避免发送到公网；不会配置系统或浏览器 DNS'
  }
